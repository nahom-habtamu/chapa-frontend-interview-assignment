...
const mockLoginUsers = mockUsers
  .filter((user) => !user.isDeactivated && user.isActive)
  .map((user) => ({
    ...user,
    password:
...
export const getCurrentUser = async (token: string): Promise<User> => {
  ...
  if (user.isDeactivated) throw new Error("User account has been deactivated");
  return user;
};
...
export const loginUser = async (
  ...
  const expectedPassword =
    user.role === "super_admin" ? "super123" : user.role === "admin" ? "admin123" : "user123";
